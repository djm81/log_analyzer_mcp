...
def test_logger_setup_create_logger_preserve_test_format(temp_log_file, mock_log_record):
    ...
    assert not isinstance(console_handler.formatter, MessageFlowFormatter) # Explicitly check it's NOT MessageFlowFormatter
...
def test_project_root_and_logs_base_dir_paths():
    ...
    assert os.path.isdir(LOGS_BASE_DIR) or not os.path.exists(LOGS_BASE_DIR), \
