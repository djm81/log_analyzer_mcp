...
def test_search_all_custom_options(runner, mock_analysis_engine_class, mock_analysis_engine_instance):
    ...
    result = runner.invoke(cli, [
        "search", "all",
        "--scope", custom_scope,
        "--before", str(custom_before),
        "--after", str(custom_after),
        "--log-dirs", custom_log_dirs,
        "--log-patterns", custom_log_patterns
    ])
...
@pytest.mark.parametrize("time_args, expected_criteria_updates", [
...
def test_search_time_various_units(runner, mock_analysis_engine_class, mock_analysis_engine_instance, time_args, expected_criteria_updates):
